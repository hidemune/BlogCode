...
class Keyboard():
	...
	def __init__(self):
		...
		while have_dev==False:
			try:
				...
				#we're only plugging one thing in cat /proc/bus/input/devices
				ret = subprocess.check_output("cat /proc/bus/input/devices | grep Handlers= | grep kbd", shell=True)
				print(ret)
				args = ret.decode("utf-8").split(" ")
				print(args[-2])
				self.dev = InputDevice("/dev/input/" + args[-2])
				have_dev=True
			except OSError as ex:
				print ex
				print "Keyboard not found, waiting 1 seconds and retrying"
				time.sleep(1)
	...
	def event_loop(self):
		...
		for event in self.dev.read_loop():
			...
			print "EV:" + str(ecodes.EV_KEY)
			print "Code:" + str(event.code)
			if event.type==ecodes.EV_KEY and event.value < 2:
				print "Send:" + str(ecodes.EV_KEY)
